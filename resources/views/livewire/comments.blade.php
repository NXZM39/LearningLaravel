<div class="flex justify-center">
    <div class="w-6/12">
        <h1 class="my-10 test-3xl">Comments</h1>
        ...
        <div>
             @if (session()->has('message'))
                <div class="p-3 bg-green-300 text-white-800 rounded shadow-sm">
                     {{ session('message') }}
                </div>
            @endif
        </div>
        ...
        <form class="my-4 flex" wire:submit.prevent="addComment">
            <input type="text" class="w-full rounded border shadow p-2 mr-2 my-2" placeholder="what's in your mind" wire:model.debounce.500ms="newComment">
                <div class="py-2">
                <button type="submit" class="p-2 bg-blue-500 w-20 roundedd shadow text-white">Add</button>
            </div>
        </form>
        ...
        <div class="rounded border shadow p-3 my-2">
            <div class="flex justify-between my-2">
                <div class="flex">
                    <p class="font-bold test-lg">{{$comment->creator->name}}</p>
